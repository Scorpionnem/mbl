...
static void	parse_face(mbl::render::Mesh& mesh, std::istringstream& iss, std::vector<vec3f>& pos_vec, std::vector<vec3f>& normal_vec, std::vector<vec2f>& uv_vec, const vec4f& current_uv_off)
{
	struct FaceVertex
	{
		int pos_index;
	...
	for (size_t i = 1; i + 1 < face_indices.size(); ++i)
	{
		...
		mbl::loader::mesh::obj::Vertex	triangle[3] =
		{
			{
				.pos = pos1,
				.normal = normal1,
...
static void	loadMTL(const std::string& obj_path, std::map<std::string, std::string>& mtl_tex_path, mbl::render::TextureAtlas &atlas, std::istringstream& iss)
{
	std::string	mtllib;
	if (!(iss >> mtllib))
...
void	mbl::loader::mesh::obj::load(const std::string& path, mbl::render::Mesh& mesh, mbl::render::TextureAtlas& atlas)
{
	std::ifstream file(path);
	if (!file.is_open())
		throw std::runtime_error("Failed to open model " + path);
	...
	mesh.add_vertex_layout(2, 2, GL_FLOAT, offsetof(mbl::loader::mesh::obj::Vertex, uv));
