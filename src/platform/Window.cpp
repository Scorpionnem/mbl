...
void    mbl::platform::Window::open(const char *title, u32 size)
{
	open(title, size, size);
}

void    mbl::platform::Window::open(const char *title, u32 width, u32 height)
{
	if (_window)
		throw std::runtime_error("mbl::platform::Window::open: already open");
...
void    mbl::platform::Window::close()
{
    if (_glContext)
    {
...
const mbl::platform::Input&    mbl::platform::Window::pollEvents()
{
...
void    mbl::platform::Window::swapBuffers()
{
    SDL_GL_SwapWindow(_window);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

void    mbl::platform::Window::captureMouse(bool captured)
{
	SDL_SetRelativeMouseMode(captured ? SDL_TRUE : SDL_FALSE);
}

bool    mbl::platform::Window::isMouseCaptured() const
{
	return (SDL_GetRelativeMouseMode() == SDL_TRUE);
}

bool    mbl::platform::Window::isOpen() const
{
	return (_window != nullptr);
}

u32     mbl::platform::Window::width() const
{
	return (_width);
}

u32     mbl::platform::Window::height() const
{
	return (_height);
}

void    mbl::platform::Window::_initSDL()
{
	if (SDL_WasInit(SDL_INIT_VIDEO))
		return ;
...
void    mbl::platform::Window::_createWindow(const char *title, u32 width, u32 height)
{
	int	mx;
	int	my;
...
void    mbl::platform::Window::_setGLAttributes()
{
...
void    mbl::platform::Window::_createGLContext()
{
